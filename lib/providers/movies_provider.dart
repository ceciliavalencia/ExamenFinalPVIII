...
class MoviesProvider extends ChangeNotifier {
  ...
  Future<List> getLogos(int id) async {
    ...
    final response = await http.get(url);
    final infoResponse = ImagesCompany.fromJson(response.body);

    return infoResponse.logos;
  }

  Future<Companydetail> getInfoCompany(int idCompany) async {
    var url = Uri.https(this._baseUrl, '3/company/${idCompany}', {
      'api_key': this._apiKey,
    });

    final response = await http.get(url);
    final companyResponse = Companydetail.fromJson(response.body);
